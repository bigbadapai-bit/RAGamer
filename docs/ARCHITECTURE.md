...
- **会话可以删掉**（左栏每条末尾那个 `✕`，`POST /chat/{库}/{会话}/delete`）：**不二次确认**——一次会话就是一段问答，删错了重问一遍就是；本仓要二次确认的是删库那种不可逆地清掉语料的动作。按钮是真的表单而不是链接，没脚本时也删得掉，也免得被浏览器预取。删的只有会话本身：澄清反问的待答记录不挂在会话上，缓存按问题存而不按会话存，两者都不用连带清理。
