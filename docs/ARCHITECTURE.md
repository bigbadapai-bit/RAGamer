...
提问走 `GET /api/chat/sessions/{id}/ask`（给 `EventSource` 留的路，它只会发 GET），流上是**四种事件**：`citations`（最先，一次）→ `delta`（若干）→ `done` 收尾；检索与生成的失败发生在开流之后，只能发 `error` ——响应头在第一个事件之前就发出去了，状态码改不了，所以**客户端靠「有没有收到 `done`」区分答完与中断**。
