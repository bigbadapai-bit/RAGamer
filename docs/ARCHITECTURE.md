...
**已定的边界**：生产级 + 面试导向，可以取舍但不能做成玩具。参照 [RAGFlow](https://github.com/infiniflow/ragflow) 的**设计思路**，不参照它的代码规模（它的前端实测 1730 文件 / 20 万行业务代码，见 [ADR-0005](./adr/0005-htmx-frontend.md)）。
...
**转机**：VLM 后端的 `image_analysis` 默认开启，而本项目用的就是 `model_version: vlm`。但有三处必须处理：
...
**与原项目共存**：共享 Milvus / MinIO / Mongo 实例，但**用 Milvus 的 database 隔离**（上限 64），而不是 collection 名前缀 —— 字符串前缀更容易撞车。MinIO 换桶名、Mongo 换库名同理。原项目须保持可运行（它是面试素材）。
...
|---|---|---|
| `chunk_id` | INT64 | 自增主键 |
| `content` | VARCHAR | 正文。图片的 VLM 摘要与二次 OCR 文字都在这里 |
...
**显式声明全部字段，关闭动态字段** —— 不是随手写的，理由见第八节「必须继承的坑」第 1 条。
...
| 长文本列 | **降级为 metadata 字段，不进正文** |
| Infobox | 原子化，不切 |

🔴 **长文本列不进正文**这条不是优化而是必须：RAGFlow 的 Table 模板没有任何 token 上限，超长行会在 embedding 阶段被 `truncate(c, mdl.max_length - 10)` **静默截断** —— 内容永久丢失且无任何报错。把"说明"这类长列设成 metadata 可以规避。
...
这是整个改造里性价比最高的一处，也是最能讲的一个点：*"我没有新增一次路由调用，而是把一个已有的 LLM 调用从输出 2 个字段扩到 3 个。"*
...
**不做** chunk 可视化编辑。**删库要清理四处**：Milvus collection + MinIO 前缀 + Mongo 会话 + Redis 缓存。
...
**在文档里写明数据来源与许可** —— 这在面试里是加分项，说明有合规意识。
...
这个项目相对开源生态的位置：**游戏 RAG 领域没有能直接用的框架** —— 除 `rimulu030/gamewiki`（308★）外全部在 45★ 以下，绝大多数是 0–3★ 的课程项目。**没有任何一个做到原项目的程度**（BGE-M3 单模型双向量 + Milvus 混合检索 + 断崖截断 + 澄清反问 + VLM 摘要进正文）。

**几个值得主动讲的取舍**：

1. **为什么一个游戏一个 collection** —— 主流建议是共享 collection + 标量过滤，Qdrant 官方称之为反模式；但那些反面案例都在千级规模，我们在几十个量级，且真正的收益是**生命周期**而非性能（[ADR-0002](./adr/0002-one-collection-per-game.md)）
2. **为什么主体标签是两个正交字段** —— 单字段的问题是这些类不在同一个轴上，会互相打架（[ADR-0003](./adr/0003-two-orthogonal-tag-fields.md)）
3. **为什么查询路由不增加 LLM 调用** —— 把已有节点的输出从 2 个字段扩到 3 个
4. **为什么缓存用 exact match 不做语义缓存** —— 归一化已经由改写节点完成，再加一层就是三层阈值互相耦合
5. **为什么前端不用 React** —— 真实约束是"出错时能否定位"，不是"能否写出来"（[ADR-0005](./adr/0005-htmx-frontend.md)）
6. **为什么代码重写而不复用** —— 可复用的只有 16%，而代价是继承一整套债务（[ADR-0001](./adr/0001-rewrite-business-layer.md)）

**"如果要重做一遍会怎么做"**：先建评测集再调参。原项目所有参数（阈值、权重、`k`、`chunk_size`）都是凭经验拍的，**没有任何数据能证明它们是最优的** —— 这是本项目把评测列为独立阶段的直接原因。
