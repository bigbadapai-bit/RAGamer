...
**一批导入最多 4 条同时跑（`BATCH_WORKERS`），限频一秒都不放宽**：并行的是「等」——MinerU 解析、抓网页、视觉摘要、打标都在等网络。限频那把锁按主机分（`crawl._throttle`），两个线程不会读到同一个「上次时刻」各自睡完同时发出去（测试钉住了这一条：不加锁时两个线程同时在睡）；不同主机之间互不等待。本地重活仍是串行的（向量化与二次 OCR 各一把锁）：CPU 机器上并发只是把同一块 CPU 切来切去，中间结果的内存还按并发数翻倍。
...
**网址来源的标题带上地址里的页码** —— 同站的分页攻略每一页标题逐字相同（实测游民星空那份图文攻略：三页的 HTML 标题与正文一级标题完全一样），而 `(doc_title, version)` 是替换范围，一批十页只活得下来一页、留下哪一页还取决于线程调度。标识因此必须**从这一条自己算出来**：地址末尾形如 `_6` 的页码进标题（`…/1803231_6.shtml` → 「…（6）」），读不出页码的就不加。按「这一批里第几个」编号看着更简单，但那个号随同批伙伴的有无而变——单独重导一页会拿到 1 号，把原来那一份删掉。
...
- **理解结果在进程内记一份**（`ragamer.query.UnderstandingMemo`）。上面那句「精确匹配就能拿到很高的命中率」有个前提：改写对同一句话得是稳定的。它其实不稳——温度已经钉死在 0，模型在 0 下仍会抖，实测同一个问题问 6 次得到 3 种改写——于是同一句话问两次会算成两个键、双双未命中，各花掉一次完整检索。记下理解结果把这个前提补上了，顺带省掉每次提问那一次模型调用。**只记成功的那些**：降级交回的是「按原问法继续、游戏与版本留空」，记下来会把一次模型抖动钉成整个进程生命周期里的固定行为。键里连着历史与候选一起算，理由见那个类。
...
| 2 | 知识库管理 | 建库（选游戏 + 配术语映射 + 勾选启用的主体类）、**列已导入的资料**、删一份资料、删库 |
...
**删一份资料清三处**（`ragamer.knowledge.purge_document`，与删库同一处、同一种「一处失败不跳过其余」）：这一份的切片 → 它的原图 → 缓存按前缀清一遍（`invalidate`，**不清提问计数**：库还在，热门问题该留着）。切片按「标题 + 版本」精确匹配，同标题的另一个版本不动（ADR-0004）。知识库配置与会话都不动——**会话是历史**，删库那条路才清它。原图只删这一份引用、**而库里别的切片没有再引用**的那些：对象 key 里那层摘要来自来源本身（文件的字节摘要、网址的哈希），同一个来源被导成两份标题不同的文档时两份指着同一批 key。要问「别人引用到哪些」得让存储层带 `excluding` 一次扫完（`ChunkStore.image_keys`）——拿「全库有哪些」减去自己那批会把两份**共用**的 key 一起减掉，共用图于是被当成没人用而删掉。删除入口只在知识库配置页那一行上，**切分预览页没有**（那一页的定位是只读）。
...
🔴 **整轮一起落库、收完正文才写**。生成中途断掉（模型炸了、用户点了停止）时什么都不写：历史里既不会留下半句答案，也不会留下一条等不到回复的提问。半截答案配一份完整的引用列表，指向的是正文里根本没写到的来源。

**这一轮归会话，不归那条 SSE**（`ragamer.live`）。跑它的线程不属于任何请求：页面断开、刷新、切走都只停转发，答案照跑完、照落库——回来刷新就看得到它。要真收手得走 `POST /api/chat/sessions/{id}/cancel`，那一轮才不写进会话。**同一个会话同时只允许一轮**：两轮各自读同一份会话、各自追加一轮存回去，后写的那份会把先写的整个盖掉，而两边的答案都真实生成过。

⚠️ **收手不是抢占式的**。跑那一轮的线程要到下一个检查点才停得下来，检查点之间可能隔着一次模型往返、一次向量化或者一次精排（实测那次精排最长二十几秒）。检查点因此埋在四处：理解之后、检索每一路开跑之前、生成之前、以及**生成的每一片之间**——最后这一处让「正在吐字」时点停止几乎立刻见效。

**切回来接着看**是这一层的另一半。`/chat/{库}` 不带会话时落在**这个库最近活跃的那一个**：离开对话页之后别的地方不知道该带哪个会话（左侧导航是那一页渲染的，它手里没有这个信息），落到最近那一个，回来的就还是刚才那一页。而正在跑的那一轮由 `GET /api/chat/sessions/{id}/live` 交出一份**快照**（`ragamer.live.TurnSnapshot`）：状态、已经生成的正文、引用，以及**问的那句**——那一轮没落库，历史里没有它。页面轮询它，跑完那一刻改用历史渲染。

⚠️ **用轮询而不是接上原来那条 SSE**。接上意味着这一轮要为「后来的人」留一份事件日志并支持多个订阅者，`LiveTurn` 就得从「一条队列」变成「日志 + 广播」；而快照那点信息量正好是页面要的全部，与导入页看进度（`ragamer.jobs` 的任务快照）也是同一个姿势。
...
- **会话可以删掉**（左栏每条末尾那个 `✕`，`POST /chat/{库}/{会话}/delete`）：**不二次确认**——一次会话就是一段问答，删错了重问一遍就是；本仓要二次确认的是删库、删一份资料那种不可逆地清掉语料的动作。按钮是真的表单而不是链接，没脚本时也删得掉，也免得被浏览器预取。删的只有会话本身：澄清反问的待答记录不挂在会话上，缓存按问题存而不按会话存，两者都不用连带清理。
...
- **正文按一个安全子集排版**（`ragamer.web.markdown`）：模型写的是 Markdown，而模板原先直接输出纯文本，`**`、`- `、`#` 全原样露在页面上。只认粗体、列表（含子列表）、标题、段落这四种，**先整段转义再放行白名单**——输出里能当标签用的只有渲染器自己插的那几种。模型输出按不可信对待是有理由的：网络检索的内容会进提示词，能诱导它吐 HTML。认不出的记号（表格、链接、行内代码）原样留着，认一半比不认更乱。**流式那一轮的正文仍是原样文本**，答完被服务端渲染好的整块换掉（`turns.html` 与它渲染的是同一份数据，页面里不拼第二遍）。
