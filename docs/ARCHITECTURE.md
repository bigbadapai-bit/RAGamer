...
> **本项目落地方式（T20）**：业务层是自写的（ADR-0001），没有引入 LangGraph，也就没有 `interrupt()` 这个机制。可暂停/恢复换成两个端点加一条记录：`POST /api/ask` 判不准时写一条**待澄清记录**（Mongo 的 `pending_questions`）并把候选交回去，`POST /api/ask/{pending_id}` 带着用户点的那个候选回来继续。上面那条 🔴 换个位置成立：**写只发生在暂停的那一刻**，恢复那条路上一个写操作都没有，所以重复恢复结果一致、也不会多写出记录。将来在恢复路径上接缓存（§4）时那条写必须幂等。
>
> 另有两处与本文的差异一并记在这里：**确定度由联合输出一并给出**（§3.1 的零额外调用），不是单独判一次；**一次只问一个维度，且先问游戏**——版本候选是按游戏取的，游戏没定就无从谈版本。
