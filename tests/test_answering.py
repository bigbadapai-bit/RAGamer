"""生成：把检索到的资料交给模型，产出带引用的答案。

三个用例组：**缝合**（预置一批候选，断言引用的顺序与条数——这是本票的验收口径）、
**来源**（引用带得出文档标题与祖先标题路径）、**没有资料时**（明确回复，不硬编）。
模型接 `FakeLlm`（一次网络都不发），检索那一段接内存假件与确定性假件。

引用是**答案可核对的前提**：答案里写了什么，得能顺着编号找回原文。所以这里断言的
不只是「有几条引用」，还有顺序（编号与资料顺序一一对应）与标签（标题 + 祖先标题路径）。
"""
...
#: 四份资料：前三份与问题高度重合，第四份只有「二郎神」三个字重合。
...
def test_预置一批候选时引用的顺序与条数():
    """断崖截断掉的那条不进引用：引用与交给生成的资料是同一批、同一个顺序。"""
    llm = FakeLlm(REPLY)

    answer = answerer(store_with(*BOSS_CHUNKS), llm).answer(QUESTION, game_id=GAME, version="1.0")
...
def test_引用的编号与提示词里的资料一一对应():
    """正文里的 [2] 要能顺着编号找回第二条资料。"""
    llm = FakeLlm(REPLY)

    answerer(store_with(*BOSS_CHUNKS), llm).answer(QUESTION, game_id=GAME, version="1.0")
...
def test_附加文本也进提示词():
    ...
    store = store_with(make_chunk(1, content="二郎神怎么打", content_meta="| 说明 | 先定身 |"))
    llm = FakeLlm(REPLY)
...
def test_生成温度钉零():
    """答案要照着资料写，不追求多样性；温度 0 才能让同一个问题两次问出同一个答案。"""
    llm = FakeLlm(REPLY)
...
def test_库里没有相关内容时给明确回复():
    """**不硬编一个答案**：没有资料可依据时不调模型——让它自由发挥只会得到一段
...
def test_版本过滤把候选滤空时也给明确回复():
    store = store_with(make_chunk(1, content=QUESTION, version="2.0"))
    llm = FakeLlm()
...
def test_空问题当场报错():
    ...
    with pytest.raises(ValueError):
        answerer(store_with(*BOSS_CHUNKS), FakeLlm(REPLY)).answer("   ", game_id=GAME)
