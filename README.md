...
- **确定度与取值成对回来**：确定不了时要反问而不是猜（§3.4），而「确定不了」不是非黑即白。
  确定度是**模型自报的 0~1**，量纲与两档阈值一起写在 `ragamer.clarifying` 里。
...
系统拿不准时反问，而不是猜（`ragamer.clarifying`，`docs/ARCHITECTURE.md` §3.4）。
读取侧的两个端点是 `POST /api/chat` 与 `POST /api/chat/{pending_id}`：前者要么给答案、
要么给一次反问（响应里的 `kind` 说清是哪种），后者是用户点完候选之后的那一次请求。
**候选的按钮由对话页渲染**（排在后面的界面票），这一层只把候选交出来。

- **候选必须来自语料里真实存在的取值，绝不由模型生成**。游戏候选是知识库列表
  （显示名给人看，id 拿去检索；两个库重名时标签带上 id），版本候选是这个库里真实出现过的
  版本（`ChunkStore.versions`）。让模型自由生成澄清选项，它会给出库里根本没有的选项，
  用户选了也检索不到——而「选了没东西」与「选了查得到」在界面上长得一样。
- **确定度分两档**（§3.4 坑 #13）：过 `CONFIDENT` 直接作答，落在 `UNSURE` 与 `CONFIDENT`
  之间是「接近但不肯定」，停下来问。两个阈值与模型自报的确定度一样是经验值，
  本项目还没有评测集（§11）。
- **没判出版本不构成反问**：版本有默认值（知识库的现行版本，ADR-0004），游戏没有。
  候选只有一个时也不问——答案已经被候选定死了，问一句只是多一次点击。游戏那一维再收一道：
  模型判出的那个与页面/会话里选定的那个**一致**时也不问，只有**对不上**才是真的拿不准。
- **一次只问一个维度，且先问游戏**——版本候选是按游戏取的。所以版本反问成立的前提是
  游戏已经定下来（页面/会话给的那个，或模型判出来且调用方给了库）。只带着问题来
  （不传 `game_id`）时给不出版本候选，模型也就判不出版本，按知识库的现行版本走。
- **恢复时不重复产生副作用**（§3.4 的坑）：原项目用 `interrupt()`，恢复时节点从头重跑，
  节点内的落库因此必须外提或幂等。这里没有那个机制，但同一条约束换个位置成立——
  **写只发生在暂停的那一刻**（待澄清记录写一条），恢复这条路上一个写操作都没有，
  所以恢复多少次结果都一致。
- 版本候选按游戏取：不知道是哪款游戏就给不出候选，只带问题来的提问因此判不出版本，
  按知识库的现行版本走。模型判出的游戏与候选来源那个库不是同一个时，它那条版本判断作废
  ——拿 A 库的版本列表去认 B 库的版本，认出来也不作数。
...
src/ragamer/          应用代码（config 配置装载、logging 日志、llm 语言模型适配器、sources 归一化、
                      ...
                      query 提问理解、retrieval 主检索路、answering 生成与引用、clarifying 澄清反问、
