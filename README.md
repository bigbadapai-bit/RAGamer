...
| `/import` | 导入：选库、**传文件（md／txt／PDF／图片，可多选）与填网址（可多条）在同一次提交里**、标注版本；逐条给出状态与走过的阶段，失败的可只重试失败的那些，跑完报出这批进了多少切片、覆盖到哪些标签 |
...
- **导入页的两条输入是同一次提交的两半**：文件与网址一起提交，编号连着排，逐条各给状态与
  「走到哪个阶段」。导入是同步的一整段，请求回来时它已经跑完——**运行中只有日志看得见进度**，
  页面上能说的是这一条走过了哪几步。失败的那几条可以只重试它们：网址原样带回，**文件带不了**
  （字节在浏览器那边，提交完就不在页面上了），得重新选中。
