...
- **这一步失败不阻断提问**：模型挂了就按原问法继续，游戏、版本与问题类型都留空交回给
  调用方；被模型服务拒绝（密钥、模型名配错）另按 ERROR 报出来，不然线上只看得见「降级了」。

## 查询路由

`ragamer.routing` 按问题类型决定这次走哪几路召回，不做无差别全跑。
...
- **没接上的路跳过但留痕**：六条路现在接了两条，选中了还没接上的那条会记一条 warning 再跳过；
  一条都不剩时退回主检索路，免得这一类问题一条候选都取不到。
...
- **已接上的两路召回**：主检索路（混合检索）与**元数据过滤路**（按主体类型、内容性质、
  版本直接取候选，单路稠密检索——再叠一路稀疏会把标签之外的近义内容也捞进来，正好抵消过滤）。
  其余四路（多查询改写、HyDE、结构化表格路、联网兜底）在后面几张票里接。
...
src/ragamer/          应用代码（config 配置装载、logging 日志、llm 语言模型适配器、sources 归一化、
                      ...
                      routing 查询路由、retrieval 多路召回与融合、answering 生成与引用、
                      conversations 会话与流式、
