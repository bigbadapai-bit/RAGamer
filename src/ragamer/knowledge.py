...
读取侧有两处读这份集合，但读的不是同一部分，所以只有一处走这个模块：

- 澄清反问要**可选的游戏**（`ragamer.clarifying`）：候选得是库里真有的游戏，
  而「库里有哪些游戏」就是这份集合里的 id。显示名与现行版本一并读出来
  ——按钮上摆显示名，检索按现行版本。**这一处就是本模块**。
- 导入端点要**打标词表**（`ragamer.api`）：`subject_types` 与 `term_mapping` 原样交给
  `ragamer.tagging`，那边认的是载荷本身。多一层转成这里的类型，只会多一处键名对不上的机会。
