...
- **组合是数据**。默认表照 §3.1 摆，知识库里配了 `route_table` 就整行覆盖。姿势与
  ...
  词表，配置只能在这两者之间连线。
- **跑不了的路跳过，但不静默**。「还没实现」与「依赖没配」（这一路要语言模型或外部检索
  服务，而这次没给）都算跑不了。选中了跑不了的那条要留一条 warning 再跳过：静默跳过会
  让「这一路这次跑不了」与「组合配错了」在日志里长得一模一样，而处置方式相反。
...
#: `ragamer.retrieval` 里真正实现的那几支对齐。写成「全部」就少了一道提醒，
#: 而漏掉的表现是「路由表里配了那条路，却一条候选都没多出来」——不报错。
#:
#: 加一条路要一起改的地方，都在 `ragamer.retrieval` 里，一处一个理由：
#: 这份清单、`_paths_to_run` 的依赖表（这一路要模型还是要检索服务）、
#: `_plans` 的取文本那一段（要不要生成）、`_search` 的查询那一段（往存储要什么）。
