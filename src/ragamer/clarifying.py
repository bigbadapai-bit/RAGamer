...
class Clarifier:
    ...
    def start(
        ...
        return self._answer(understanding.rewritten_query, resolved_game, resolved_version)

    def resume(self, pending_id: str, label: str) -> Answer:
        """从暂停点继续：用户点的那一项补进那次判定，然后照常作答。

        **这一步只读不写**（见模块说明）：重复调用得到同一份答案，也不会多出一条记录。
    ...
    def _answer(self, query: str, game_id: str, version: str) -> Answer:
        """交给 `Answerer`。现行版本现读一次——它是「当前该用哪个版本」的唯一真相来源。"""
        base: KnowledgeBase | None = knowledge_base(self.docs, game_id)
