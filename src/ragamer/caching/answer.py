...
class CachedAnswerer:
    ...
    def _key(
        ...
        *,
        ...
        🔴 键取的是**改写后的问法**，而改写**不保证两次一样**：温度已经钉死在 0
        （`ragamer.query.TEMPERATURE`），但推理模型在 0 下仍会抖——实测同一个问题问
        6 次得到 3 种改写。所以同一个问题问两次**有可能算成两个键、双双未命中**，
        热门问题里也会被拆成几条。这是权衡之后接受的取舍：换用原问题算键能稳，
        代价是「那它怎么打」这类追问不再与直接问命中同一条。

        **别把这里的抖动当成 bug 去修**：它是模型的性质。真嫌命中率低，要动的是这个
        取舍本身，而不是给 `normalize_query` 加规则——那个函数同时被用来比候选，
        收紧它会顺带收紧候选的比对（见它的说明）。
        """
