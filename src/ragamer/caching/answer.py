...
from ragamer.answering import Answer, Answerer, StreamingAnswer
...
class CachedAnswerer:
    ...
    def stream(
        ...
        *,
        ...
        cached = None if key is None else self._read(key, question)
        if cached is not None:
            return StreamingAnswer(cached.citations, cached.images, replay(cached.text))
        streamed = self.answers.stream_answer(
            question, game_id=game_id, version=version, current_version=current_version
        )
    ...
    def _written_back(
        self, key: str | None, streamed: StreamingAnswer, question: str
    ) -> Iterator[str]:
        """边吐边攒，吐完写回缓存。**没吐完不写**：半个答案进缓存比不缓存更糟——
        下一次命中它，用户拿到的是一段断掉的话，而且看不出这是缓存给的。
        """
        pieces: list[str] = []
        for piece in streamed.text:
            pieces.append(piece)
            yield piece
        if key is None:
            return
